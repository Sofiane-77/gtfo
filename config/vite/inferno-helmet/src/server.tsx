...
const generateTitleAsString = (
  ...
  return `<title ${HELMET_ATTRIBUTE}="true"${attrsStr}>${encodeSpecialCharacters(
    flattenedTitle,
    encode,
  )}</title>`;
};
...
function generateTagsAsString<T>(
  ...
  for (const tag of tags) {
    ...
    for (const [name, value] of entries) {
      if (
        !(
          name === (TAG_PROPERTIES.INNER_HTML as string) ||
    ...
    const tagContent =
      (tag as ScriptProps).innerHTML ?? (tag as StyleProps).cssText ?? "";
...
function mapElementAttributesToProps<T>(
  ...
  ops: { addHelmetDataAttr?: boolean; addKey?: string | number } = {},
...
function renderTitle(title: string, attrs: TitleProps): InfernoNode {
  return [
    <title
      key={title}
  ...
  return {
    base: {
      ...
      toString() {
        const s = getState();
        return s.base
          ? generateTagsAsString("base", [s.base], s.encodeSpecialCharacters)
          : "";
      },
    },
    bodyAttributes: {
    ...
    priority: {
      ...
      toString() {
        const s = getState();
        const meta = generateTagsAsString(
          "meta",
          s.priority?.meta ?? [],
          s.encodeSpecialCharacters,
        );
        const link = generateTagsAsString(
          "link",
          s.priority?.links ?? [],
          s.encodeSpecialCharacters,
        );
        const script = generateTagsAsString(
          "script",
          s.priority?.script ?? [],
          s.encodeSpecialCharacters,
        );
